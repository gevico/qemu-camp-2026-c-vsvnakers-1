 ...
 */
typedef struct {
    uint16_t version_be;  /* 版本字段，网络序 */
    uint16_t length_be;   /* 长度字段，网络序 */
    uint8_t flags_raw;    /* 标志字段，原始值 */
} proto_header_raw_t;
 ...
 */
typedef struct {
    unsigned int ver_major:4;  /* 主版本号，4位 */
    unsigned int ver_minor:4;  /* 次版本号，4位 */
    unsigned int length:16;    /* 长度，16位 */
...
int main(void) {
    ...
    unsigned int ver_major = (version >> 4) & 0x0F;
    unsigned int ver_minor = version & 0x0F;
